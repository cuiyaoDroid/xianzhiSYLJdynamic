		...
		pulldownlist.setRefreshListioner(new OnRefreshListioner() {
			...
			public void onRefresh() {
				...
				if(keyword.length()==0){
					...
					getTrainListDate(0,keyword,true,false);
				}
		...
		searchEdit.setOnEditorActionListener(new OnEditorActionListener() {
			...
			public boolean onEditorAction(TextView v, int actionId,
				...
				if (actionId == EditorInfo.IME_ACTION_SEARCH) {
					...
					String keyword = searchEdit.getText().toString().trim();
					progressBar.setVisibility(View.VISIBLE);
					getTrainListDate(0,keyword,false,false);
...
//		progressBar.setVisibility(View.VISIBLE);
	...
	private void refreshMoreParolData(String keyword) {
		...
		ArrayList<DynamicListHolder> holders;
		if (keyword==null) {
			holders = helper.selectData(page.curpage* page.pagesize, page.pagesize,date);
		} else {
			holders = helper.selectSearchData(keyword, page.curpage* page.pagesize, page.pagesize,date);
		}
		helper.close();
		for (DynamicListHolder holder : holders) {
			addHolderData(holder);
