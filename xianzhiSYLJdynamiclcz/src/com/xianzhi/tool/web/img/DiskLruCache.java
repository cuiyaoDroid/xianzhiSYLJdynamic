...
package com.xianzhi.tool.web.img;
	...
	public Bitmap get(String key) {
		synchronized (mLinkedHashMap) {
			...
			if (file != null) {
				...
				Bitmap bmp;
				try {
	            	bmp = PictureUtil.getSmallBitmap(file);
				} catch (OutOfMemoryError e) {
					Log.d(TAG, "Disk cache hit OOM");
					// TODO: handle exception
					bmp = PictureUtil.getimage(file, 480, 800, 1024*1024 * 2);
				}
				return bmp;
			} else {
				...
				if (new File(existingFile).exists()) {
					...
					Bitmap bmp;
					try {
		            	bmp = PictureUtil.getSmallBitmap(existingFile);
					} catch (OutOfMemoryError e) {
						// TODO: handle exception
						Log.d(TAG, "Disk cache hit OOM");
						bmp = PictureUtil.getimage(existingFile, 480, 800, 1024*1024 * 2);
					}
					return bmp;
				}
			}
