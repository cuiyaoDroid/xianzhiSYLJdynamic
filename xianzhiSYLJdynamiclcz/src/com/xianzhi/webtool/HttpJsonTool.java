...
//	 "http://v.cc-railway.xzh-soft.com:8082/dynamic/";
	...
	public static final String updateServerUrl = "http://v.sy-railway.xzh-soft.com:8180/version/check.json";
	...
	public synchronized String checkupdata(int productId) {
		String version;
		String url;
		try {
			...
			HttpPost httpRequest = new HttpPost(updateServerUrl);
			...
			JSONObject json = jsonObject.getJSONObject("versionInfo");
	...
	private void insertTrainDynamic(Context context, JSONArray list,
		...
		synchronized (lock.Lock) {
			...
			for (int i = 0; i < list.length(); i++) {
				...
				TrainDynamicHolder holder = new TrainDynamicHolder(-1,
						train_id, STATION_NAME, STATION_NO, TIME_ARRIVAL,
	...
	public synchronized String addNewDynamicTrain(Context context, int trainid,
			String trainCode, String startTime, String currentTeam,
			String teamLength, String date, boolean isFirst) {
		try {
			HttpClient client = HttpsClient.getInstance().getHttpsClient();
			if (cookieInfo != null) {
				((AbstractHttpClient) client).setCookieStore(cookieInfo);
			...
			if (trainid == -1) {
				...
				if (isFirst) {
					httpRequest = new HttpPost(ServerUrl
							+ "/train/setTrainConductor.json?token="
							+ SecurityCheckApp.token);
				} else {
					httpRequest = new HttpPost(ServerUrl
							+ "/train/addTrainConductor.json?token="
							+ SecurityCheckApp.token);
				}
				// /train/setTrainConductor.json
			}
			List<NameValuePair> params = new ArrayList<NameValuePair>();
			if (trainid != -1) {
	...
	private void insertTrainList(Context context, JSONArray list,
			boolean ifclear, String date) throws JSONException {
	...
	private void insertUserInfo(Context context, JSONArray array)
			throws JSONException {
		UserHelper helper = new UserHelper(context);
		synchronized (lock.Lock) {
			for (int i = 0; i < array.length(); i++) {
				JSONObject json = array.getJSONObject(i);
				helper.insert(getUserHolder(json), helper.getWritableDatabase());
			}
		}
		helper.close();
	}
	...
	private UserHolder getUserHolder(JSONObject json) {
		...
		UserHolder holder = new UserHolder(id, job_number, user_name, photo,
				phone, department_id, department_name, position_id,
				position_name);
		return holder;
	}

	public synchronized String clearTrainConductor(Context context, int trainid) {
		try {
			HttpClient client = HttpsClient.getInstance().getHttpsClient();
			if (cookieInfo != null) {
				((AbstractHttpClient) client).setCookieStore(cookieInfo);
			...
			HttpPost httpRequest;
			httpRequest = new HttpPost(ServerUrl
					+ "/train/clearTrainConductor.json?token="
					+ SecurityCheckApp.token);
			List<NameValuePair> params = new ArrayList<NameValuePair>();
			if (trainid != -1) {
				params.add(new BasicNameValuePair("id", String.valueOf(trainid)));
		...
		synchronized (lock.Lock) {
			...
			for (int i = 0; i < list.length(); i++) {
				...
				TrainCoachHolder holder = new TrainCoachHolder(coach_no,
						train_no, coach_type, limit1, limit2, trainDate,
						trainCode, trainId, passenger_num, actual);
				helper.insert(holder, db);
			}
			db.setTransactionSuccessful();
			db.endTransaction();
	...
	public synchronized String getSeatInfo(Context context, String trainCode,
		...
		try {
			...
			L.i(ServerUrl + "/train/getSeatInfo.json?token="
					+ SecurityCheckApp.token);
			List<NameValuePair> params = new ArrayList<NameValuePair>();
			params.add(new BasicNameValuePair("trainCode", trainCode));
